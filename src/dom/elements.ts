 ...
 */
export const getElementOrThrow = <T extends Element>(
  source: QuerySource,
  selector: string,
): T => {
  ...
  if (!element) {
    const querySource =
      source instanceof Document ? "light DOM" : "Shadow Root";
    throw new Error(
      `Required element with selector "${selector}" was not found in ${querySource}.`,
    );
  }
  return element;
};
 ...
 */
export const getElementOrElse = <T extends Element>(
  source: QuerySource,
  selector: string,
  alternative: T,
): T => {
 ...
 */
export const querySelectorAllOrThrow = <T extends Element>(
  source: QuerySource,
  selector: string,
): NodeListOf<T> => {
  ...
  if (elements.length === 0) {
    const querySource =
      source instanceof Document ? "light DOM" : "Shadow Root";
    throw new Error(
      `Required elements with selector "${selector}" were not found in ${querySource}.`,
    );
  }
  return elements;
};
...
export const getElementByIdOrThrow = <T extends HTMLElement>(
  ...
  if (!element) {
    const querySource =
      source instanceof Document ? "light DOM" : "Shadow Root";
    throw new Error(
      `Element with ID "${id}" not found in was not found in ${querySource}`,
    );
  }
  return element as T;
};
 ...
 * @returns The found element or the alternative.
 */
export const getElementByIdOrElse = <T extends HTMLElement>(
  source: DOMSource,
  id: string,
  alternative: T,
): T => {
 ...
 * @throws Error if no elements are found.
 */
export const getElementsByClassNameOrThrow = <T extends Element>(
  source: Document | Element,
  className: string,
): HTMLCollectionOf<T> => {
  const elements = source.getElementsByClassName(
    className,
  ) as HTMLCollectionOf<T>;
  if (elements.length === 0) {
    const querySource = source instanceof Document ? "Document" : elements;
    throw new Error(
      `Elements with class name "${className}" not found in ${querySource}.`,
    );
  }
  return elements;
};
